...
| ~~**M7**~~ | ~~`figs/double_jet_natl_panels.pdf` / `.png`~~ *(retired — D4)* | 4.5 | Retired by **D4**, superseded by M10–M12. The panel code stays in the tree and stays verified: it is still the smoke run's **M6**, and `--stage figure` builds the campaign panels on demand |
| **M8** | One documented command | cli | `python -m double_jet --config configs/double_jet.yaml` reproduces **M1, M3, M4, M5** and the D4 deliverables (**M10–M12**) end to end — the one-command build now ends with the explorer; `--skip-download` reuses `$SCRATCH`. M2 and M6 are `--smoke` deliverables and M7 is retired, so none of the three is part of the campaign command |
...
| **M10** | `figs/double_jet_natl_explorer.html` — one self-contained interactive explorer, no network and no external scripts (**D4**) | D4 §B9 | nine hard failures, each naming the offender: the season's date vector, `profile_sha256` against the config, the latitude axis against `sector.resolution`, every required attribute, the season set, the three detection thresholds on every CSV row, the CSV columns, state/null consistency, and finite JSON (D4 §B11 G1–G9). `tests/test_explorer.py` round-trips every day of every season from the embedded payload back to the CSV, and asserts the HTML carries no external reference |
| **M11** | `results/season_summary.csv` — one row per season, tracked (**D4**) | D4 §B7 | `tests/test_explorer.py` recomputes all eight columns independently from the CSV with pandas and asserts row-for-row equality; the three state counts sum to `n_days`; the counts derived from the explorer's embedded payload equal the table's rows, so the HTML and the table cannot drift |
| **M12** | `figs/double_jet_natl_annual.png` — a static rendering of the explorer's top chart (**D4**) | D4 §B8 | rendered from M11's CSV, which the build writes first, so "the same numbers by construction" is literal rather than an argument; `tests/test_explorer.py` asserts the file exists, is a PNG by its magic bytes, and is non-trivial in size |
